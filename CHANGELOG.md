...
- `PlatformTray`, behind the new `system-tray` feature: a real tray icon on
  ...
  Merged from the `system-tray` branch on 14 September, after 1.2.0 was cut.
  Its CHANGELOG entry was written under `[Unreleased]` before 1.1.0 existed,
  so merging placed it inside the 1.1.0 section — a release tagged six days
  before this code reached master. Moved here, where it belongs.

### Fixed

- **The `system-tray` feature was compiled by no CI job.** A whole platform
  backend arrived declared, documented and unbuilt, which is the hole
  `ws-transport`, `derive` and `agpu-backend` were added to CI to close. It is
  on the cross-platform job now — Windows and macOS need no system packages,
  while Linux would need `libgtk-3-dev` first.

  On its first run it earned the place: `every_menu_id_maps_back_to_the_one_
  the_application_chose` builds a real platform menu, and `muda` panics inside
  itself on a headless macOS runner rather than returning an error. It is
  `#[ignore]`d with the reason and the command to run it on a machine with a
  desktop session.

- An intra-doc link in `src/tray.rs` pointed at `PlatformTray`, which exists
  only behind the feature, so the default doc build CI runs could not resolve
  it. That was the Documentation job the PR had been failing.
...
- **The protocol reference showed what to ask and, for thirteen of fifteen
  ...
  single pair. It was comparing two. The reference now shows the real reply for
  `ping`, `get_schema`, `get_state`, `execute_action`, `validate`, `subscribe`,
  `negotiate`, `get_performance` and `screenshot` — generated by running them,
  not written from memory — and the floor is eight.

- Two things that check found once it could see them:

  `ping` was documented as answering `{"status": "pong", "framework": "dewey",
  "version": "1.0.0"}`. There is no `version` field; it is `protocol_version`,
  and it is `2`. The line had been written as prose rather than as a fenced
  block, so the check never parsed it — a response example that is not JSON is
  a response example nothing holds to account.

  `negotiate`'s documented `server_capabilities` was missing `performance`,
  added to the server earlier in this cycle. The check compared keys and not
  values, so a stale list inside a correctly-named array was invisible. That
  one array is what an agent branches on, so it is now compared against
  `SERVER_CAPABILITIES` element by element.

- The comparison walks nested objects. It stopped at the top level of `data`,
  so `data.last_frame.render_ms` — a field an agent reads by name — was
