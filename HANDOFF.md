...
| `tests/agent_view.rs` | a widget that does not tell a reader what an agent can do with it, and a block that disagrees with the widget it describes. The blocks are generated from each widget's own `schema`, `actions`, `agent_state` and `capabilities`, so renaming an action fails the build until the comment follows. |
